    ...
    async def 각인(self, ctx, *engrave_name):
        ...
        engrave_diclist = {
            ...
            '광기': ('(버서커 전용) 분노 게이지가 항상 최대치로 유지되며, 폭주 시 어둠의 힘을 이용한 폭주 상태가 된다. '
                   '폭주 상태에서 적에게 주는 피해가 3%/8%/16% 증가하고, 공격 및 이동속도가 15.0% 증가하며 받는 모든 피해가 65.0% 감소한다. '
                   '하지만, 폭주 상태가 되는 순간 최대 생명력의 25.0%로 전환되며, 생명력 회복 효과가 적용되더라도 이를 초과할 수 없게 되고, 회복 효과는 40%만, 실드 효과는 25%만 적용된다. '
                   ' X키를 통해 폭주를 해제할 수 있으며 해제 즉시 최대 생명력의 25.0%를 회복하지만, 30초 동안 폭주를 할 수 없게 된다.'),

            '광전사의 비기': '(버서커 전용) 폭주 중 치명타 피해가 35%/50%/70% 증가하고 폭주 종료 시 탈진 효과가 발생하지 않는다.',

            '극의: 체술': '(인파이터 전용) 기력 에너지의 자연 회복 속도가 300%/450%/600% 증가하며 기력 스킬의 피해량이 35%/50%/65% 증가하지만 충격 스킬의 피해량이 30% 감소한다.',
            ...
            '두 번쨰 동료': ('(호크아이 전용) 실버호크 MK-II 를 소환하여 파티원의 이동속도가 추가로 4% 증가하며, '
                        ...
                        '추가로 실버호크가 소환 되어 있는 동안 자신의 공격력이 3/6/10% 증가한다.'),
            ...
            '버스트': ('(블레이드 전용) 블레이드 버스트 스킬 사용 시 보유한 블레이드 오브 개수와 상관없이 최고 단계의 블레이드 버스트가 발동된다. '
                    '해당 블레이드 버스트는 잔재된 기운 효과가 발생하지 않는다'
                    '또한 아츠 발동 중 평타 및 각성기를 제외한 스킬 적중 시 0.4초 마다 버스트 강화효과가 중첩된다. (최대 20중첩)'
                    ' 해당 효과는 블레이드 버스트 스킬의 피해량을 최대 100%/110%/120% 까지 증가시키며 중첩 당 피해량 상승폭은 중첩이 강화될수록 증가한다. '
                    '또한 매 중첩마다 공격력이 0/0.5/1% 씩 증가한다'
                    '추가로 블레이드 아츠 종료 시 보유한 버스트 강화 효과의 개수만큼 블레이드 오브 게이지를 획득한다.'),

            '분노의 망치': '(디스트로이어 전용) 해방 스킬 사용 시 소모한 코어 수에 비례하여 치명타 적중률이 3%/4%/5%, 치명타 피해량이 5%/10%/15% 증가한다.',

            '사냥의 시간': '(건슬링어 전용) 핸드건 및 라이플 스킬의 치명타 적중률이 20%/30%/40% 증가하지만 샷건 스탠스를 사용할 수 없다.',
            ...
            '심판자': '(홀리나이트 전용) 징벌 스킬의 피해량이 15%/20%/25%, 징벌 스킬 타격 시 신앙게이지 획득량이 100% 증가한다. 신의 집행자 지속 시간이 100%/125%/150% 증가한다.',
            ...
            '이슬비': '(기상술사 전용)여우비의 빗방울 게이지 소모량이 50% 감소하며, 여우비가 활성화되어 있는 동안 기상 스킬의 피해량이 7%/15%/30% 증가한다.',
            ...
            '절정': ('(창술사 전용) 듀얼 게이지 3단계가 가득 찬 상태에서 스탠스 전환 시 획득하는 난무/집중 3단계 효과 대신 강화된 (난무/집중1)/(난무/집중2)/(난무/집중3)을 획득한다.'
                   '절정: 난무 1/2/3: 공격속도가 12.5%/13.8%/15% 증가, 적에게 주는 피해가 12.5%/13.8%/15% 치명타 적중률이 20%/22.5%/25% 증가'
                   '절정: 집중 1/2/3: 이동속도가 15% 증가, 적에게 주는 피해가 15%/17.5%/20% 증가, 치명타 피해가 37.5%/43.8%/50% 증가'),

            '절제': '(창술사 전용) 집중 스탠스를 사용할 수 없지만 난무 스킬의 피해량이 18%/27%/36% 증가한다.',

            '죽음의 습격': '(호크아이 전용) 최후의 습격 스킬 사용 시 남은 호크 게이지의 50%를 돌려받으며, 적중된 대상이 자신에게 받는 피해가 8초동안 22%/33%.44% 증가한다.',

            '중력 수련': ('(디스트로이어 전용) 전투 중 중력 게이지가 초당 1.0%/1.5%/2.0%씩 자연 회복된다.'
                      '또한 중력 가중 모드 시 평타와 볼텍스 그라비티의 치명타 적중률이 10%/20%/30% 증가하며 적에게 주는 피해가 4%/10%/20% 증가한다.'),
            ...
            '질풍노도': ('(기상술사 전용) 기본 이동 속도 증가량 %의 7%/15%/30% 만큼 치명타 적중률이 증가하며, 기본 공격 속도 증가량 %의 22%/45%/90% 만큼 치명타 피해량이 증가한다.'
                     '우산 스킬을 사용하고 3초 이내에 우산 스킬을 사용하면 최대 빗방울 게이지의 12%를 회복한다. 여우비가 강화되고 우산 스킬로 적용된다.'
                     '여우비의 범위가 감소하며 빗방울 게이지 소모량이 50% 증가하고, 아군에게 더 이상 받는 피해 감소 효과를 부여하지 않는다.'
                     '대신 아군에게 공격 및 이동 속도 12% 증가 효과를 부여한다.'),

            '초심': '(배틀마스터 전용) 적에게 주는 피해가 156%/24%/32% 증가하지만, 더 이상 엘리멘탈 게이지를 획득할 수 없다.',
            ...
            '헨드거너': ('(데빌헌터 전용) 헨드건 스탠스만 사용할 수 있지만, 핸드건 스킬의 피해량이 30%/50%/70% 증가 및 무력화 피해가 40% 증가한다.'
                     ' 또한 각성기의 피해량이 15%/22%/30% 증가한다.'),

            '화력 강화': ('(블래스터 전용) 받는 피해가 20% 감소한다. '
                      '또한 화력 버프의 단계에 따라 치명타 적중률이 (15%, 20%, 25%)/(22%, 27%, 32%)/(30%, 35%, 40%) 증가한다.'),
            ...
            '오의난무': '(스트라이커 전용) 오의 스킬 사용 시 엘리멘탈 구슬을 반드시 1개만 소모하며, 오의 스킬의 피해가 8%/13%/18% 증가한다.',

            '점화': ('(소서리스 전용) 마력 해방 발동 시 각성기 및 이동기를 제외한스킬의 남은 재사용 대기시간이 50%로 감소하며,'
                   ' 마력 해방 중 치명타 적중률이 10%/17%/25%, 치명타 피해가 20%/35%/50% 증가한다.'),

            '환류': '(소서리스 전용) 마력 방출을 사용 할 수 없지만, 각성기 및 이동기를 제외한 스킬의 피해량이 8%/12%/16% 만큼 증가하고 재사용 대기시간이 3%/6%/10% 감소한다.',

            '회귀': '(도화가 전용) 저무는 달 및 떠오르는 해 스킬 사용 시, 자신에게 60초 동안 치명타 적중률 6%/9%/12% 및 치명타 피해가 20%/30%/40% 증가하는 효과를 부여한다.',

            '만개': ('(도화가 전용) 떠오르는 해 시전 시 기운이 널리 퍼져 구슬을 획득한 대상을 포함한 24m 범위 내에 있는 파티원에게 '
