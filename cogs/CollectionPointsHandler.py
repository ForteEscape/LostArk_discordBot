...
class CollectionPointHandler(commands.Cog):
    ...
    @commands.command()
    async def 내실(self, ctx, *input_target):
        # 데이터 최신화 필요
        target = ' '.join(input_target)
        ...
        if target == '':
            ...
            elif target == "오르페우스의 별" or target == "오페별":
            ...
            elif target == "위대한 미술품" or target == "미술품":
    ...
    @commands.command()
    async def 모험의서(self, ctx, *location):
        input_location = ' '.join(location)
        
        # 로웬 추가 필요
        location_dic = {'아르테미스': 'https://gam3.tistory.com/5',
                        '유디아': 'https://gam3.tistory.com/8?category=929580',
                        '루테란 서부': 'https://gam3.tistory.com/10?category=929580',
                        '루테란 동부': 'https://gam3.tistory.com/11?category=929580',
                        ...
                        '베른 북부': 'https://gam3.tistory.com/15?category=929580',
                        ...
                        '베른 남부': 'https://www.inven.co.kr/webzine/news/?news=249587&site=lostark',
                        '로웬': 'https://gam3.tistory.com/49'
                        }
        ...
        if input_location == '':
            ...
            if input_location in location_dic:
                await ctx.send(input_location + ' 모험의 서와 관련된 정보입니다\n' + location_dic[input_location])
            else:
                await ctx.send(input_location + ' 지역은 없는 지역입니다 띄어쓰기를 하였을 경우 붙여쓰기를 해주세요')
